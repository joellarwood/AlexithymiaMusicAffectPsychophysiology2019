...
PID <-  unique(joinedaffectemo$participant)

negICC <- 

  {subnegstim <- select(negstim, 
                        ANGRY, ANXIOUS, ASHAMED, FRUSTRATED, GUILTY, SAD, SCARED) 
  icc = icc(subnegstim, model = "twoway", unit = "average") #computes ICC
  icc_temp = icc$value
...
for (sub in PID){
  ...
  negICC=rbind(negICC,row_temp)
  print(fz_iccTemp)
}

colnames(negICC)=c("Participant","ICCneg","ICCneg.fz")
