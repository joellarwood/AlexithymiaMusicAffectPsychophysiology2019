...
vis_dat(survey) + theme(axis.text.x = element_text(angle = 90))
vis_miss(survey) + theme(axis.text.x = element_text(angle = 90))
```

## Recode gender
```{r, message = FALSE}
survey <- genderRecode(input=survey, # "/SurveyData.csv" if reproducing
...
survey %>% select(EOTlist) %>% alpha()
survey %>% select(DDFlist) %>% alpha()
survey %>% select(DIFlist) %>% alpha()
survey %>% select(EOTlist, DDFlist, DIFlist) %>% alpha()
```
...
survey %>% select(stresslist) %>% alpha(na.rm = TRUE)
survey %>% select(anxietylist) %>% alpha()
survey %>% select(depressionlist) %>% alpha()
```
...
# Describe data 
```{r}
select(joined, arousalkey.keys, valencekey.keys, TAS) %>% describe() %>% kable()
```
...
for(i in participant){
  ...
  stim_select <- select(participant_select, poslist)
  icc <- irr::icc(stim_select, model = "twoway", unit = "average")
  icc_temp <- icc$value
  fz_iccTemp <- fisherz(icc_temp)
  row_temp <- c(i, icc_temp, fz_iccTemp)
...
colnames(icc_list_posstim)=c("participant","ICCpos","ICCpos.fz")
...
joinedICCfilter <- drop_na(joinedICC)
```
