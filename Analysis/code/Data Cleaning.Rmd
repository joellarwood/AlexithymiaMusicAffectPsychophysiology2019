...
# Survey Data 
```{r}
survey <- read_csv(file = "../data/SurveyData.csv")
vis_dat(survey) 
vis_miss(survey) 
...
Mean trials completed per participant were  `r print(nrow(wordspread)/max(trials$participant))`
...
scores <- survey %>% select(participant, depression, EOT, DDF, DIF, TAS, gender_recode, musichours, yearsplay)
